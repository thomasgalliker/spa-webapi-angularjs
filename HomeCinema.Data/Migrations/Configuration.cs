...
namespace HomeCinema.Data.Migrations
{
    internal sealed class Configuration : DbMigrationsConfiguration<HomeCinemaContext>
    {
        ...
        {
            //  create genres
            context.GenreSet.AddOrUpdate(g => g.Name, this.GenerateGenres());

            // create movies
            context.MovieSet.AddOrUpdate(m => m.Title, this.GenerateMovies());

            //// create stocks
            context.StockSet.AddOrUpdate(this.GenerateStocks());
        ...
        private Stock[] GenerateStocks()
        {
            ...
            for (int i = 1; i <= 15; i++)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    Stock stock = new Stock { MovieId = i, UniqueKey = Guid.NewGuid(), IsAvailable = true };
                    stocks.Add(stock);
                }
            }
